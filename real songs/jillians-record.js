...
samples('bubo:fox')
const synth_energy = slider(1922, 500,5000)
const chop_energy = mul(synth_energy, 1.25)

//JILLIAN

$: s("youngandnauseous:1")
.slow(4)
.chop(16).cut(1)
.sometimesBy(.5, ply("2"))
.sometimesBy(.25, mul(speed("-1")))
.gain(1.5).room(2).rsize(.5).delay(.2)
.ribbon("[11|7|15]", 1).hpf(200)
...
_$: s("white!16").decay(sine.fast(8).range(0.01, 0.05)).gain(.7).room(0.25).late("[.004 .006]*2")
     .velocity("[1 .8]*4")
     .sometimesBy(0.15, x=>x.ply("2"))

$: note("<e1 b1 f#1 f#1>").s("supersaw")
  .seg(8)
...
$: note("<d#4(3,8,0)!2 [b4(3,8,0)!1|d#4(4,16,16)|d#4(4,16,16)e4(3,16,16)]>".add(note('0,.20,-.20')))
  .s("supersaw")
  .fast(2)
  .transpose("12,0,-12")
  .lpf(synth_energy)
...
_$: s("youngandnauseous").slow(16).gain(3).slice(8, "0 1 2 3 4 5 6 7").slow(16).room(1)
