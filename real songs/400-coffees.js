...
$: s("~ fohh:3").fast(4).speed(-2).decay(0.1).gain(1.5)._scope()


_$: s("swat").fit().chop(16).cut(1)
  .sometimesBy(.5, mul(speed("0.9")))
  .gain(.75).lpf(3500)._scope()
$: s("~ fsnare:2").fast(2).decay(0.1).gain(0.5)

$: s("400-coffees").slow(8).speed("-1|2")
   .slice(16, `< ~ [~ 4] ~ [~ 2]>`)
   .hpf(1000)
   .orbit(2)
   .gain(2).room(1)._punchcard()
$: s("400-coffees").slow(8)
  .slice(16, `
  <[2 3] [2 3] [2 3] [~ 4]
   [5!2] [6 7] [5!2] [~ 4]
  [12 13] [12 13] [2 3] [~ 4]
  ...
  .delay(.2)
  .orbit(2)
  .gain(2).room(0.5)._punchcard()

_$: s("400-coffees").slow(8).gain(1.25)._scope()
_$: s("400-coffees:1").slow(8).gain(1).room(1)._scope()
