...
setCps(130 / 60 / 4)
const arp_lpf_sq = slider(1000, 0, 20000)
const bass_lpf = slider(10000, 0, 20000)
const first = true
...
const chord_part = first ? '<E B F# F#>' : '<E B F# C#-7>'
let chords = chord(chord_part).dict('ireal')
const mel = stack(
  chords
    .offset(-1)
    .voicing()
    ...
    .rsize(2)
    .dist(1.25)
    .lpf(slider(7360, 0, 20000))
)

samples('github:yaxu/clean-breaks')
samples('github:algorave-dave/samples/')

const verse_one = s('giveittome')
  .n(2)
  .chop(16)
  .cut(8)
  ...
  .cpm(130 / 4)
  .lpf(slider(7500, 0, 20000))

const hat_break = s('swat')
  .fit()
  .chop(16)
  .cut(1)
...
const crate_loop = stack(
  s('bd').struct('<[x*<1 2> [~@3 x]] x>'),
  s('~ [rim:<2 2 4 3>, sd:<2 2 2 3>]').room('<0 .1 0 .2>'),
  // n("[0 <1 3>]*<2!3 2!3 2!3 4>").s("hh"),
  s('rd:<1!3 2>*2').gain(0.5)
  // s("rd:<1!4>*1").gain(.5)
)
  .cpm(130 / 2)
  .bank('crate')
